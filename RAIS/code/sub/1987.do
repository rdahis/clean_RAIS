...
foreach state in `states' {
	...
	import delimited "input/1987/`state'1987ID.txt", clear varn(1) delim(";") case(preserve) stringcols(_all)
	...
	//tempfile f`state'
	//save `f`state''
	save "tmp/1987_`state'.dta", replace

}
*

local first : word 1 of `states'
use "tmp/1987_`first'.dta", clear
foreach state in `states' {
	if "`state'" != "`first'" {
		qui append using "tmp/1987_`state'.dta", force
	}
}
*
...
save "output/data/identified/full/1987.dta", replace

foreach state in `states' {
	erase "tmp/1987_`state'.dta"
}
