...
log using "output/log/2015.log", replace

local states AC AL AM AP BA CE DF ES GO MA MG MS MT PA PB PE PI PR RJ RN RO RR RS SC SE SP TO

foreach state in `states' {
	
	cap import delimited "input/2015/`state'2015ID.txt", clear varn(1) delim(";") case(preserve) stringcols(_all)
	cap import delimited "input/2015/`state'2015ID.TXT", clear varn(1) delim(";") case(preserve) stringcols(_all)
	...
	destring municipio tipoadm tpvinculo causadesli empem3112 mesdesli diadesli idade grinstrucao sexotrabalhador tamestab tipoestbl horascontr indceivinc tiposal indalvara indpat indsimples portdefic tpdefic raca_cor qtdiasafas causafast* anochegbr, replace force
	...
	*cap replace diadeslig="0" if diadeslig=="NAO DESL ANO"
	*destring diadeslig, replace
	*recode diadeslig (0 = 0 "Nao desligado"), pre(n)label(diadeslig)
	*drop diadeslig
	*ren ndiadeslig diadeslig
	
	recode mesdesli (0 = 0 "Nao desligado"), pre(n)label(mesdesli)
	drop mesdesli
	ren nmesdesli mesdesli
	
	replace diadesli = 0 if diadesli == .
	...
	*
	...
	recode natjuridica	(1015 = 1015 "POD EXEC FED") (1023 = 1023 "POD EXEC EST") ///
						...
						(1210 = 1210 "ASSOC PUBLIC") (2013 = 2013 "ADM PUB OUTR") ///
						(2013 = 2013 "EMP PUB") (2020 = 2020 "EMP PB SA CP") ///
	...
	//tempfile f`state'
	//save `f`state''
	save "tmp/2015_`state'.dta", replace

}
*

local first : word 1 of `states'
use "tmp/2015_`first'.dta", clear
foreach state in `states' {
	if "`state'" != "`first'" {
		qui append using "tmp/2015_`state'.dta", force
	}
}
*
...
save "output/data/identified/full/2015.dta", replace

foreach state in `states' {
	erase "tmp/2015_`state'.dta"
}
