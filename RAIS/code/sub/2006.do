...
log using "output/log/2006.log", replace

local states centro06 nordeste06 norte06 sp06 sp106 sudeste06 sul06

foreach state in `states' {
	
	import delimited "input/2006/`state'.TXT", clear varn(1) delim(";") case(preserve) stringcols(_all)
	...
	destring municipio tipoadm tpvinculo causadesli empem3112 mesdesli grinstrucao tamestab tipoestbl horascontr indceivinc tiposal indalvara indpat indsimples portdefic tpdefic raca_cor, replace force
	...
	*
	...
	recode natjuridica	(1015 = 1015 "POD EXEC FED") (1023 = 1023 "POD EXEC EST") ///
						...
						(1210 = 1210 "ASSOC PUBLIC") (2006 = 2006 "ADM PUB OUTR") ///
	...
	save "tmp/2006_`state'.dta", replace

}
*

local first : word 1 of `states'
use "tmp/2006_`first'.dta", clear
foreach state in `states' {
	if "`state'" != "`first'" {
		qui append using "tmp/2006_`state'.dta", force
	}
}
*
...
save "output/data/identified/full/2006.dta", replace

foreach state in `states' {
	erase "tmp/2006_`state'.dta"
}
