...
log using "output/log/2011.log", replace

local states AC AL AM AP BA CE DF ES GO MA MG MS MT PA PB PE PI PR RJ RN RO RR RS SC SE SP TO

foreach state in `states' {
	
	cap import delimited "input/2011/`state'2011ID.txt", clear varn(1) delim(";") case(preserve) stringcols(_all)
	cap import delimited "input/2011/`state'2011ID.TXT", clear varn(1) delim(";") case(preserve) stringcols(_all)
	...
	*
	...
	recode natjuridica	(1015 = 1015 "POD EXEC FED") (1023 = 1023 "POD EXEC EST") ///
						...
						(1210 = 1210 "ASSOC PUBLIC") (2011 = 2011 "ADM PUB OUTR") ///
	...
	//tempfile f`state'
	//save `f`state''
	save "tmp/2011_`state'.dta", replace

}
*

local first : word 1 of `states'
use "tmp/2011_`first'.dta", clear
foreach state in `states' {
	if "`state'" != "`first'" {
		qui append using "tmp/2011_`state'.dta", force
	}
}
*
...
save "output/data/identified/full/2011.dta", replace

foreach state in `states' {
	erase "tmp/2011_`state'.dta"
}
