...
log using "output/log/1990.log", replace

local states AC AL AM AP BA CE DF ES GO MA MG MS MT PA PB PE PI PR RJ RN RO RR RS SC SE SP TO

foreach state in `states' {
	
	import delimited "input/1990/`state'1990ID.txt", clear varn(1) delim(";") case(preserve) stringcols(_all)
	...
	//tempfile f`state'
	//save `f`state''
	save "tmp/1990_`state'.dta", replace

}
*

local first : word 1 of `states'
use "tmp/1990_`first'.dta", clear
foreach state in `states' {
	if "`state'" != "`first'" {
		qui append using "tmp/1990_`state'.dta", force
	}
}
*
...
save "output/data/identified/full/1990.dta", replace

foreach state in `states' {
	erase "tmp/1990_`state'.dta"
}
