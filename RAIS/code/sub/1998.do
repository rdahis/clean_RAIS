...
foreach state in `states' {
	...
	//tempfile f`state'
	//save `f`state''
	save "tmp/1998_`state'.dta", replace

}
*

local first : word 1 of `states'
use "tmp/1998_`first'.dta", clear
foreach state in `states' {
	if "`state'" != "`first'" {
		qui append using "tmp/1998_`state'.dta", force
	}
}
*
...
save "output/data/identified/full/1998.dta", replace

foreach state in `states' {
	erase "tmp/1998_`state'.dta"
}
