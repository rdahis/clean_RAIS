...
foreach state in `states' {
	...
	*
	...
	//tempfile f`state'
	//save `f`state''
	save "tmp/2017_`state'.dta", replace

}
*

local first : word 1 of `states'
use "tmp/2017_`first'.dta", clear
foreach state in `states' {
	if "`state'" != "`first'" {
		qui append using "tmp/2017_`state'.dta", force
	}
}
*
...
save "output/data/identified/full/2017.dta", replace

foreach state in `states' {
	erase "tmp/2017_`state'.dta"
}
