...
foreach state in `states' {
	...
	cap import delimited "input/1996/`state'1996ID.txt", clear varn(1) delim(";") case(preserve) stringcols(_all)
	cap import delimited "input/1996/`state'1996ID.TXT", clear varn(1) delim(";") case(preserve) stringcols(_all)
	cap import delimited "input/1996/`state'1996id.txt", clear varn(1) delim(";") case(preserve) stringcols(_all)
	...
	recode natjuridica	(1015 = 1015 "POD EXEC FED") (1023 = 1023 "POD EXEC EST") ///
						...
						(1210 = 1210 "ASSOC PUBLIC") (1996 = 1996 "ADM PUB OUTR") ///
	...
	//tempfile f`state'
	//save `f`state''
	save "tmp/1996_`state'.dta", replace

}
*

local first : word 1 of `states'
use "tmp/1996_`first'.dta", clear
foreach state in `states' {
	if "`state'" != "`first'" {
		qui append using "tmp/1996_`state'.dta", force
	}
}
*
...
save "output/data/identified/full/1996.dta", replace

foreach state in `states' {
	erase "tmp/1996_`state'.dta"
}
