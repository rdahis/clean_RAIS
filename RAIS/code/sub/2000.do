...
log using "output/log/2000.log", replace

local states AC AL AM AP BA CE DF ES GO MA MG MS MT PA PB PE PI PR RJ RN RO RR RS SC SE SP TO

foreach state in `states' {
	
	import delimited "input/2000/`state'2000ID.txt", clear varn(1) delim(";") case(preserve) stringcols(_all)
	...
	destring municipio uf anoadm tpvinculo causadesli empem3112 mesdesli grinstrucao sexotrabalhador tamestab tipoestbl mesadmissao idade natjuridica horascontr indceivinc, replace force
	...
	*
	...
	recode natjuridica	(1015 = 1015 "POD EXEC FED") (1023 = 1023 "POD EXEC EST") ///
						...
						(1210 = 1210 "ASSOC PUBLIC") (2000 = 2000 "ADM PUB OUTR") ///
	...
	//tempfile f`state'
	//save `f`state''
	save "tmp/2000_`state'.dta", replace

}
*

local first : word 1 of `states'
use "tmp/2000_`first'.dta", clear
foreach state in `states' {
	if "`state'" != "`first'" {
		qui append using "tmp/2000_`state'.dta", force
	}
}
*
...
save "output/data/identified/full/2000.dta", replace

foreach state in `states' {
	erase "tmp/2000_`state'.dta"
}
