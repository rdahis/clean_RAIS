...
foreach state in `states' {
	...
	if "`state'" == "SP1" {
		import delimited "input/2010/SP2010ID1.TXT", clear varn(1) delim(";") case(preserve) stringcols(_all)
	}
	else if "`state'" == "SP2" {
		import delimited "input/2010/SP2010ID2.TXT", clear varn(1) delim(";") case(preserve) stringcols(_all)
	}
	else {
		import delimited "input/2010/`state'2010ID.TXT", clear varn(1) delim(";") case(preserve) stringcols(_all)
	}
	*
	...
	*
	...
	recode natjuridica	(1015 = 1015 "POD EXEC FED") (1023 = 1023 "POD EXEC EST") ///
						...
						(1210 = 1210 "ASSOC PUBLIC") (2010 = 2010 "ADM PUB OUTR") ///
...
use "tmp/2010_`first'.dta", clear
foreach state in `states' {
	if "`state'" != "`first'" {
		qui append using "tmp/2010_`state'.dta", force
	}
}
*
...
save "output/data/identified/full/2010.dta", replace

foreach state in `states' {
	erase "tmp/2010_`state'.dta"
}
