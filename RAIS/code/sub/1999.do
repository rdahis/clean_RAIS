...
log using "output/log/1999.log", replace

local states AC AL AM AP BA CE DF ES GO MA MG MS MT PA PB PE PI PR RJ RN RO RR RS SC SE SP TO

foreach state in `states' {
	
	cap import delimited "input/1999/`state'1999ID.txt", clear varn(1) delim(";") case(preserve) stringcols(_all)
	cap import delimited "input/1999/`state'1999ID.TXT", clear varn(1) delim(";") case(preserve) stringcols(_all)
	...
	*
	...
	recode natjuridica	(1015 = 1015 "POD EXEC FED") (1023 = 1023 "POD EXEC EST") ///
						...
						(1210 = 1210 "ASSOC PUBLIC") (1999 = 1999 "ADM PUB OUTR") ///
	...
	save "tmp/1999_`state'.dta", replace

}
*

local first : word 1 of `states'
use "tmp/1999_`first'.dta", clear
foreach state in `states' {
	if "`state'" != "`first'" {
		qui append using "tmp/1999_`state'.dta", force
	}
}
*
...
save "output/data/identified/full/1999.dta", replace

foreach state in `states' {
	erase "tmp/1999_`state'.dta"
}
