...
foreach state in `states' {
	...
	*
	...
	//tempfile f`state'
	//save `f`state''
	save "tmp/2013_`state'.dta", replace

}
*

local first : word 1 of `states'
use "tmp/2013_`first'.dta", clear
foreach state in `states' {
	if "`state'" != "`first'" {
		qui append using "tmp/2013_`state'.dta", force
	}
}
*
...
save "output/data/identified/full/2013.dta", replace

foreach state in `states' {
	erase "tmp/2013_`state'.dta"
}
