...
foreach state in `states' {
	...
	//tempfile f`state'
	//save `f`state''
	save "tmp/1994_`state'.dta", replace

}
*

local first : word 1 of `states'
use "tmp/1994_`first'.dta", clear
foreach state in `states' {
	if "`state'" != "`first'" {
		qui append using "tmp/1994_`state'.dta", force
	}
}
*
...
save "output/data/identified/full/1994.dta", replace

foreach state in `states' {
	erase "tmp/1994_`state'.dta"
}
