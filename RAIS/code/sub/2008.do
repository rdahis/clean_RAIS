...
foreach state in `states' {
	...
	*
	...
	save "tmp/2008_`state'.dta", replace

}
*

local first : word 1 of `states'
use "tmp/2008_`first'.dta", clear
foreach state in `states' {
	if "`state'" != "`first'" {
		qui append using "tmp/2008_`state'.dta", force
	}
}
*
...
save "output/data/identified/full/2008.dta", replace

foreach state in `states' {
	erase "tmp/2008_`state'.dta"
}
