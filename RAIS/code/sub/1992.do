...
foreach state in `states' {
	...
	import delimited "input/1992/`state'1992ID.txt", clear varn(1) delim(";") case(preserve) stringcols(_all)
	...
	save "tmp/1992_`state'.dta", replace

}
*

local first : word 1 of `states'
use "tmp/1992_`first'.dta", clear
foreach state in `states' {
	if "`state'" != "`first'" {
		qui append using "tmp/1992_`state'.dta", force
	}
}
*
...
save "output/data/identified/full/1992.dta", replace

foreach state in `states' {
	erase "tmp/1992_`state'.dta"
}
