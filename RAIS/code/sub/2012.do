...
foreach state in `states' {
	...
	cap import delimited "input/2012/`state'2012ID.txt", clear varn(1) delim(";") case(preserve) stringcols(_all)
	cap import delimited "input/2012/`state'2012ID.TXT", clear varn(1) delim(";") case(preserve) stringcols(_all)
	...
	*
	...
	recode natjuridica	(1015 = 1015 "POD EXEC FED") (1023 = 1023 "POD EXEC EST") ///
						...
						(1210 = 1210 "ASSOC PUBLIC") (2012 = 2012 "ADM PUB OUTR") ///
						(2012 = 2012 "EMP PUB") (2020 = 2020 "EMP PB SA CP") ///
...
use "tmp/2012_`first'.dta", clear
foreach state in `states' {
	if "`state'" != "`first'" {
		qui append using "tmp/2012_`state'.dta", force
	}
}
*
...
save "output/data/identified/full/2012.dta", replace

foreach state in `states' {
	erase "tmp/2012_`state'.dta"
}
