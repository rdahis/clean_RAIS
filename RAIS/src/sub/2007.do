...
log using "output/log/2007.log", replace

local states centooeste07 nordeste07 norte07 sp07 sudeste07 sul07

foreach state in `states' {
	
	import delimited "input/2007/`state'.TXT", clear varn(1) delim(";") case(preserve) stringcols(_all)
	...
	destring municipio tipoadm clascnae95 clascnae20 sbclas20 tpvinculo causadesli empem3112 mesdesli grinstrucao tamestab tipoestbl horascontr indceivinc tiposal indalvara indpat indsimples portdefic tpdefic raca_cor causafast*, replace force
	...
	*
	...
	replace diainiaf1="-1" if diainiaf1=="IGNORADO"
	destring diainiaf1, replace
	replace diafimaf1="-1" if diafimaf1=="IGNORADO"
	destring diafimaf1, replace
	replace diainiaf2="-1" if diainiaf2=="IGNORADO"
	destring diainiaf2, replace
	replace diafimaf2="-1" if diafimaf2=="IGNORADO"
	destring diafimaf2, replace
	replace diainiaf3="-1" if diainiaf3=="IGNORADO"
	destring diainiaf3, replace
	replace diafimaf3="-1" if diafimaf3=="IGNORADO"
	...
	save "tmp/2007/`state'.dta", replace

}
*

local first : word 1 of `states'
use "tmp/2007/`first'.dta", clear
foreach file in `states' {
	if "`file'" != "`first'" {
		qui append using "tmp/2007/`file'.dta", force
	}
}
*

save "output/data/full/RAIS_2007.dta", replace
