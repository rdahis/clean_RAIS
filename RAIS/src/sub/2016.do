...
foreach state in `states' {
	...
	destring municipio clascnae20 sbclas20 tipoadm tpvinculo causadesli empem3112 diadesli mesdesli idade grinstrucao sexotrabalhador tamestab tipoestbl horascontr indceivinc tiposal indalvara indpat indsimples portdefic tpdefic raca_cor qtdiasafas causafast* anochegbr ibgesubsetor muntrab, replace force
	...
	*
	...
	save "tmp/2016/`state'.dta", replace

}
*

local first : word 1 of `states'
use "tmp/2016/`first'.dta", clear
foreach file in `states' {
	if "`file'" != "`first'" {
		qui append using "tmp/2016/`file'.dta", force
	}
}
*

save "output/data/full/RAIS_2016.dta", replace
