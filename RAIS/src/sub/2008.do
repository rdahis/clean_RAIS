...
log using "output/log/2008.log", replace
...
foreach state in `states' {
	...
	if "`state'" == "SP1" {
		import delimited "input/2008/sp08.TXT", clear varn(1) delim(";") case(preserve) stringcols(_all)
	}
	else if "`state'" == "SP2" {
		import delimited "input/2008/sp08 1.TXT", clear varn(1) delim(";") case(preserve) stringcols(_all)
	}
	else {
		import delimited "input/2008/`state'2008ID.TXT", clear varn(1) delim(";") case(preserve) stringcols(_all)
	}
	*
	...
	cap ren GRINSTRUCAO	grinstrucao
	cap ren GRINSTR		grinstrucao
	cap ren GRAUINSTR	grinstrucao
	...
	destring municipio clascnae95 clascnae20 sbclas20 tipoadm tpvinculo causadesli empem3112 mesdesli grinstrucao tamestab tipoestbl horascontr indceivinc tiposal indalvara indpat indsimples portdefic tpdefic raca_cor qtdiasafas causafast*, replace force
	...
	*
	...
	recode natjuridica	(1015 = 1015 "POD EXEC FED") (1023 = 1023 "POD EXEC EST") ///
						...
						(1210 = 1210 "ASSOC PUBLIC") (2008 = 2008 "ADM PUB OUTR") ///
	...
	save "tmp/2008/`state'.dta", replace

}
*

local first : word 1 of `states'
use "tmp/2008/`first'.dta", clear
foreach file in `states' {
	if "`file'" != "`first'" {
		qui append using "tmp/2008/`file'.dta", force
	}
}
*

save "output/data/full/RAIS_2008.dta", replace
