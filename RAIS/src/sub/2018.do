...
foreach state in `states' {
	...
	destring municipio clascnae20 sbclas20 tipoadm tpvinculo causadesli empem3112 diadesli mesdesli idade grinstrucao sexotrabalhador tamestab tipoestbl horascontr indceivinc tiposal indalvara indpat indsimples indtrabparc indtrabint portdefic tpdefic raca_cor qtdiasafas causafast* anochegbr ibgesubsetor muntrab, replace force
	// indsindic
	...
	*
	...
	save "tmp/2018/`state'.dta", replace
...
use "tmp/2018/`first'.dta", clear
foreach file in `states' {
	if "`file'" != "`first'" {
		qui append using "tmp/2018/`file'.dta", force
	}
}
*

save "output/data/full/RAIS_2018.dta", replace
