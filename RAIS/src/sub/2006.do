...
foreach state in `states' {
	...
	destring municipio tipoadm clascnae95 clascnae20 sbclas20 tpvinculo causadesli empem3112 mesdesli grinstrucao tamestab tipoestbl horascontr indceivinc tiposal indalvara indpat indsimples portdefic tpdefic raca_cor, replace force
	...
	*
	...
	save "tmp/2006/`state'.dta", replace

}
*

local first : word 1 of `states'
use "tmp/2006/`first'.dta", clear
foreach file in `states' {
	if "`file'" != "`first'" {
		qui append using "tmp/2006/`file'.dta", force
	}
}
*

save "output/data/full/RAIS_2006.dta", replace
