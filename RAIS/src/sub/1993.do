...
foreach state in `states' {
	...
	save "tmp/1993/`state'.dta", replace

}
*

local first : word 1 of `states'
use "tmp/1993/`first'.dta", clear
foreach file in `states' {
	if "`file'" != "`first'" {
		qui append using "tmp/1993/`file'.dta", force
	}
}
*

save "output/data/full/RAIS_1993.dta", replace
