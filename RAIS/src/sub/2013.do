...
log using "output/log/2013.log", replace

local states AC AL AM AP BA CE DF ES GO MA MG MS MT PA PB PE PI PR RJ RN RO RR RS SC SE SP TO

foreach state in `states' {
	
	cap import delimited "input/2013/`state'2013ID.txt", clear varn(1) delim(";") case(preserve) stringcols(_all)
	cap import delimited "input/2013/`state'2013ID.TXT", clear varn(1) delim(";") case(preserve) stringcols(_all)
	...
	*
	...
	order PIS CPF numectps nome identificad radiccnpj municipio ///
		...
		ocup2002 grinstrucao genero idade nacionalidad portdefic tpdefic raca_cor ///
	...
	save "tmp/2013/`state'.dta", replace

}
*

local first : word 1 of `states'
use "tmp/2013/`first'.dta", clear
foreach file in `states' {
	if "`file'" != "`first'" {
		qui append using "tmp/2013/`file'.dta", force
	}
}
*
...
save "output/data/full/RAIS_2013.dta", replace
