...
foreach state in `states' {
	...
	destring municipio clascnae95 tipoadm tpvinculo causadesli empem3112 mesdesli grinstrucao tamestab tipoestbl horascontr indceivinc tiposal indalvara indpat indsimples portdefic raca_cor, replace force
	...
	*
	...
	save "tmp/2005/`state'.dta", replace

}
*

local first : word 1 of `states'
use "tmp/2005/`first'.dta", clear
foreach file in `states' {
	if "`file'" != "`first'" {
		qui append using "tmp/2005/`file'.dta", force
	}
}
*

save "output/data/full/RAIS_2005.dta", replace
