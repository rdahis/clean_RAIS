...
foreach state in `states' {
	...
	*
	...
	destring municipio clascnae95 causadesli empem3112 mesdesli grinstrucao sexotrabalhador tamestab tipoestbl natjuridica horascontr indceivinc tiposal indalvara indpat indsimples, replace force
	...
	*
	...
	order PIS CPF nome identificad radiccnpj municipio tpvinculo empem3112 dtadmissao causadesli mesdesli ///
		ocupacao94 grinstrucao genero dtnascimento nacionalidad remdezembro remmedia remdezr remmedr tempempr ///
		clascnae95 tamestab tipoestbl natjuridica
	
	save "tmp/2002/`state'.dta", replace

}
*

local first : word 1 of `states'
use "tmp/2002/`first'.dta", clear
foreach file in `states' {
	if "`file'" != "`first'" {
		qui append using "tmp/2002/`file'.dta", force
	}
}
*

save "output/data/full/RAIS_2002.dta", replace
