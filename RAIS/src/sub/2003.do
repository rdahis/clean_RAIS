...
log using "output/log/2003.log", replace

local states Centro_Oeste2003 Nordeste2003 Norte2003 sp2003 sudeste Sul2003

foreach state in `states' {
	
	import delimited "input/2003/`state'.txt", clear varn(1) delim(";") case(preserve) stringcols(_all)
	...
	destring municipio clascnae95 tipoadm tpvinculo causadesli diadesli empem3112 mesdesli grinstrucao sexotrabalhador tamestab tipoestbl natjuridica horascontr indceivinc tiposal indalvara indpat indsimples portdefic raca_cor, replace force
	...
	foreach i of varlist remdezembro remmedia remdezr remmedr tempempr ultrem salcontr {
		gen `i'_ = subinstr(`i',",",".",.)
	...
	*
	...
	recode natjuridica	(1015 = 1015 "POD EXEC FED") (1023 = 1023 "POD EXEC EST") ///
						...
						(1210 = 1210 "ASSOC PUBLIC") (2003 = 2003 "ADM PUB OUTR") ///
	...
	order PIS CPF numectps nome identificad radiccnpj municipio ///
		...
		ocupacao94 ocup2002 grinstrucao genero dtnascimento nacionalidad portdefic raca_cor ///
	...
	save "tmp/2003/`state'.dta", replace

}
*

local first : word 1 of `states'
use "tmp/2003/`first'.dta", clear
foreach file in `states' {
	if "`file'" != "`first'" {
		qui append using "tmp/2003/`file'.dta", force
	}
}
*

save "output/data/full/RAIS_2003.dta", replace
