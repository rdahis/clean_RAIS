...
log using "output/log/2017.log", replace

local states AC AL AM AP BA CE DF ES GO MA MG MS MT PA PB PE PI PR RJ RN RO RR RS SC SE SP TO

foreach state in `states' {
	
	import delimited "input/2017/`state'2017ID.txt", clear varn(1) delim(";") case(preserve) stringcols(_all)
	...
	ren IndTrabParcial			indtrabparc
	ren IndTrabIntermitente		indtrabint
	ren IndSindical				indsindic
	...
	destring municipio clascnae20 sbclas20 tipoadm tpvinculo causadesli empem3112 diadesli mesdesli idade grinstrucao sexotrabalhador tamestab tipoestbl horascontr indceivinc tiposal indalvara indpat indsimples indtrabparc indtrabint indsindic portdefic tpdefic raca_cor qtdiasafas causafast* anochegbr ibgesubsetor muntrab, replace force
	...
	destring diadesli, replace force
	...
	foreach v in indalvara indpat indsimples indtrabparc indtrabint indsindic {
		recode `v' (0 = 0 "Nao") (1 = 1 "Sim"), pre(n)label(`v')
		drop `v'
		ren n`v' `v'
	}
	*
	
	cap replace ocupacao94="" if ocupacao94=="000-1"
	...
	*
	...
	recode natjuridica	(1015 = 1015 "POD EXEC FED") (1023 = 1023 "POD EXEC EST") ///
						...
						(1210 = 1210 "ASSOC PUBLIC") (2017 = 2017 "ADM PUB OUTR") ///
						(2017 = 2017 "EMP PUB") (2020 = 2020 "EMP PB SA CP") ///
	...
	la var indtrabparc		"Indicador de trabalho parcial"
	la var indtrabint		"Indicador de trabalho intermitente"
	la var indsindic		"Indicador de sindicalizacao"
	...
	order PIS CPF numectps nome identificad radiccnpj municipio ///
		...
		ocupacao94 ocup2002 grinstrucao genero dtnascimento idade nacionalidad portdefic tpdefic raca_cor ///
	...
	save "tmp/2017/`state'.dta", replace

}
*

local first : word 1 of `states'
use "tmp/2017/`first'.dta", clear
foreach file in `states' {
	if "`file'" != "`first'" {
		qui append using "tmp/2017/`file'.dta", force
	}
}
*

save "output/data/full/RAIS_2017.dta", replace
