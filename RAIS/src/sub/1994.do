...
log using "output/log/1994.log", replace

local states AC AL AM AP BA CE DF ES GO MA MG MS MT PA PB PE PI PR RJ RN RO RR RS SC SE SP TO

foreach state in `states' {
	
	import delimited "input/1994/`state'1994ID.txt", clear varn(1) delim(";") case(preserve) stringcols(_all) // encoding("utf-8")
	...
	//destring anoadm tpvinculo causadesli, replace force
	destring municipio uf anoadm tpvinculo causadesli empem3112 mesdesli grinstrucao sexotrabalhador tamestab tipoestbl mesadmissao idade ibgeatividade natjuridica horascontr, replace force
	...
	save "tmp/1994/`state'.dta", replace

}
*

local first : word 1 of `states'
use "tmp/1994/`first'.dta", clear
foreach file in `states' {
	if "`file'" != "`first'" {
		qui append using "tmp/1994/`file'.dta", force
	}
}
*

save "output/data/full/RAIS_1994.dta", replace
