...
foreach state in `states' {
	...
	destring municipio uf clascnae95 anoadm tpvinculo causadesli empem3112 mesdesli grinstrucao sexotrabalhador tamestab tipoestbl mesadmissao idade natjuridica horascontr indceivinc indalvara indpat indsimples, replace force
	...
	*
	...
	save "tmp/2001/`state'.dta", replace

}
*

local first : word 1 of `states'
use "tmp/2001/`first'.dta", clear
foreach file in `states' {
	if "`file'" != "`first'" {
		qui append using "tmp/2001/`file'.dta", force
	}
}
*

save "output/data/full/RAIS_2001.dta", replace
