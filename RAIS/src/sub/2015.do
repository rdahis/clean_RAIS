...
foreach state in `states' {
	...
	destring municipio clascnae20 sbclas20 tipoadm tpvinculo causadesli empem3112 mesdesli diadesli idade grinstrucao sexotrabalhador tamestab tipoestbl horascontr indceivinc tiposal indalvara indpat indsimples portdefic tpdefic raca_cor qtdiasafas causafast* anochegbr, replace force
	...
	*
	...
	save "tmp/2015/`state'.dta", replace

}
*

local first : word 1 of `states'
use "tmp/2015/`first'.dta", clear
foreach file in `states' {
	if "`file'" != "`first'" {
		qui append using "tmp/2015/`file'.dta", force
	}
}
*

save "output/data/full/RAIS_2015.dta", replace
