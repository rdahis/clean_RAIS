...
foreach state in `states' {
	...
	destring municipio tipoadm tpvinculo causadesli diadesli empem3112 mesdesli grinstrucao sexotrabalhador tamestab tipoestbl natjuridica horascontr indceivinc tiposal indalvara indpat indsimples portdefic raca_cor, replace force
...
use "tmp/2003_`first'.dta", clear
foreach state in `states' {
	if "`state'" != "`first'" {
		qui append using "tmp/2003_`state'.dta", force
	}
}
*
...
save "output/data/identified/full/2003.dta", replace

foreach state in `states' {
	erase "tmp/2003_`state'.dta"
}
