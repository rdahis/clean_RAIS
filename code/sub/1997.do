...
log using "output/log/1997.log", replace

local states AC AL AM AP BA CE DF ES GO MA MG MS MT PA PB PE PI PR RJ RN RO RR RS SC SE SP TO

foreach state in `states' {
	
	cap import delimited "input/1997/`state'1997ID.txt", clear varn(1) delim(";") case(preserve) stringcols(_all)
	cap import delimited "input/1997/`state'1997ID.TXT", clear varn(1) delim(";") case(preserve) stringcols(_all)
	...
	recode natjuridica	(1015 = 1015 "POD EXEC FED") (1023 = 1023 "POD EXEC EST") ///
						...
						(1210 = 1210 "ASSOC PUBLIC") (1997 = 1997 "ADM PUB OUTR") ///
	...
	save "tmp/1997_`state'.dta", replace

}
*

local first : word 1 of `states'
use "tmp/1997_`first'.dta", clear
foreach state in `states' {
	if "`state'" != "`first'" {
		qui append using "tmp/1997_`state'.dta", force
	}
}
*
...
save "output/data/identified/full/1997.dta", replace

foreach state in `states' {
	erase "tmp/1997_`state'.dta"
}
