...
log using "output/log/2005.log", replace

local states centro05 nordeste05 norte05 sp05 sp105 sudeste05 sul05

foreach state in `states' {
	
	import delimited "input/2005/`state'.TXT", clear varn(1) delim(";") case(preserve) stringcols(_all)
	...
	destring municipio tipoadm tpvinculo causadesli empem3112 mesdesli grinstrucao tamestab tipoestbl horascontr indceivinc tiposal indalvara indpat indsimples portdefic raca_cor, replace force
	...
	replace diadesli="0" if diadesli=="NAO DESL ANO"
	...
	*
	...
	recode natjuridica	(1015 = 1015 "POD EXEC FED") (1023 = 1023 "POD EXEC EST") ///
						...
						(1210 = 1210 "ASSOC PUBLIC") (2005 = 2005 "ADM PUB OUTR") ///
	...
	save "tmp/2005_`state'.dta", replace

}
*

local first : word 1 of `states'
use "tmp/2005_`first'.dta", clear
foreach state in `states' {
	if "`state'" != "`first'" {
		qui append using "tmp/2005_`state'.dta", force
	}
}
*
...
save "output/data/identified/full/2005.dta", replace

foreach state in `states' {
	erase "tmp/2005_`state'.dta"
}
