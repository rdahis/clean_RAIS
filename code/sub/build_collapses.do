...
if "${SAMPLE}" == "FALSE" {
	...
	*
	...
	use "output/data/collapsed/RAIS_${FIRST_YEAR}_collapsed_estab.dta", clear
	foreach year of numlist ${SECOND_YEAR}(1)${LAST_YEAR} {
		append using "output/data/collapsed/RAIS_`year'_collapsed_estab.dta"
	}
	*
	
	save "output/data/collapsed/RAIS_${FIRST_YEAR}-${LAST_YEAR}_collapsed_establishment.dta", replace
...
if "${SAMPLE}" == "FALSE" {
	...
	*
	...
	use "output/data/collapsed/RAIS_${FIRST_YEAR}_collapsed_establishment.dta", clear
	foreach year of numlist ${SECOND_YEAR}(1)${LAST_YEAR} {
