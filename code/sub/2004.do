...
log using "output/log/2004.log", replace

local states centro_oeste nordeste Norte SP sudeste sul

foreach state in `states' {
	
	import delimited "input/2004/`state'.TXT", clear varn(1) delim(";") case(preserve) stringcols(_all)
	...
	destring municipio tipoadm tpvinculo causadesli diadesli empem3112 mesdesli grinstrucao sexotrabalhador tamestab tipoestbl horascontr indceivinc tiposal indalvara indpat indsimples portdefic raca_cor, replace force
	...
	*
	...
	recode natjuridica	(1015 = 1015 "POD EXEC FED") (1023 = 1023 "POD EXEC EST") ///
						...
						(1210 = 1210 "ASSOC PUBLIC") (2004 = 2004 "ADM PUB OUTR") ///
	...
	save "tmp/2004_`state'.dta", replace

}
*

local first : word 1 of `states'
use "tmp/2004_`first'.dta", clear
foreach state in `states' {
	if "`state'" != "`first'" {
		qui append using "tmp/2004_`state'.dta", force
	}
}
*
...
save "output/data/identified/full/2004.dta", replace

foreach state in `states' {
	erase "tmp/2004_`state'.dta"
}
