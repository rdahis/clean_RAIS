...
log using "output/log/1985.log", replace

local states AC AL AM AP BA CE DF ES GO MA MG MS MT PA PB PE PI PR RJ RN RO RR RS SC SE SP //TO

foreach state in `states' {
	
	import delimited "input/1985/`state'1985ID.txt", clear varn(1) delim(";") case(preserve) stringcols(_all)
...
use "tmp/1985_`first'.dta", clear
foreach state in `states' {
	if "`state'" != "`first'" {
		qui append using "tmp/1985_`state'.dta", force
	}
}
*
...
save "output/data/identified/full/1985.dta", replace

foreach state in `states' {
	erase "tmp/1985_`state'.dta"
}
