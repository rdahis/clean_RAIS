...
log using "output/log/2001.log", replace

local states AC AL AM AP BA CE DF ES GO MA MG MS MT PA PB PE PI PR RJ RN RO RR RS SC SE SP TO

foreach state in `states' {
	
	cap import delimited "input/2001/`state'2001ID.txt", clear varn(1) delim(";") case(preserve) stringcols(_all)
	cap import delimited "input/2001/`state'2001ID.TXT", clear varn(1) delim(";") case(preserve) stringcols(_all)
	...
	destring municipio uf anoadm tpvinculo causadesli empem3112 mesdesli grinstrucao sexotrabalhador tamestab tipoestbl mesadmissao idade natjuridica horascontr indceivinc indalvara indpat indsimples, replace force
	...
	foreach i of varlist remdezembro remmedia remdezr remmedr tempempr {
		gen `i'_ = subinstr(`i',",",".",.)
	...
	*
	...
	recode natjuridica	(1015 = 1015 "POD EXEC FED") (1023 = 1023 "POD EXEC EST") ///
						...
						(1210 = 1210 "ASSOC PUBLIC") (2001 = 2001 "ADM PUB OUTR") ///
	...
	la var anoadm			"Ano de admissao do trabalhador"
	...
	save "tmp/2001_`state'.dta", replace

}
*

local first : word 1 of `states'
use "tmp/2001_`first'.dta", clear
foreach state in `states' {
	if "`state'" != "`first'" {
		qui append using "tmp/2001_`state'.dta", force
	}
}
*
...
save "output/data/identified/full/2001.dta", replace

foreach state in `states' {
	erase "tmp/2001_`state'.dta"
}
